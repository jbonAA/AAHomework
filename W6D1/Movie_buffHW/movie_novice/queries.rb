...
top_title = Movie.select('movies.id', 'movies.title')
top_title.where('score >= ?', 9)
...
def star_wars
  ...
  # hint: use 'select' and 'where'
  star_wars_films = Movie.select('movies.id', 'movies.title', 'movies.yr')
  star_wars_films.where(title: 'Star Wars%')
